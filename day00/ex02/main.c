...
int main(void)
{
	...
	printf("price for the art \'%s\' is %d\n", "Sunflowers", searchPrice(arts, n, "Sunflowers"));
	printf("price for the art \'%s\' is %d\n", "The Potato Eaters", searchPrice(arts, n, "The Potato Eaters"));
	printf("price for the art \'%s\' is %d\n", "Kiss", searchPrice(arts, n, "Kiss"));
	printf("price for the art \'%s\' is %d\n", "American Gothic", searchPrice(arts, n, "American Gothic"));
	printf("price for the art \'%s\' is %d\n", "Nighthawks", searchPrice(arts, n, "Nighthawks"));
	printf("price for the art \'%s\' is %d\n", "Night Watch", searchPrice(arts, n, "Night Watch"));
	printf("price for the art \'%s\' is %d\n", "!", searchPrice(arts, n, "!"));
	int i = 0;
	while (arts[i])
	{
		if (searchPrice(arts, n, arts[i]->name) == -1)
			printf("Error: Art not found!!!%s, %d\n", arts[i]->name, arts[i]->price);
		i++;
	}
	printf("All artwork in the database has been found!\n");
	return (0);
}
